...
# select data
covid <- select(data, long, lat, casesw, tmean, r.mean, ah, rh, uv, kgclzone, oxgov10r, r.sd) %>% mutate(prec = 1 / (r.sd^2))
...
p1 <- mutate(covid, kgclzone = factor(kgclzone, LETTERS[1:4],
  ...
  scale_size(breaks = round(br$brks), 
             range = c(2, 7)) +
  guides(
    size = "none",
...
ggsave("Fig2.png", width = 8.42, height = 5.97)
