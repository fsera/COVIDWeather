...
## COVID cases
# clases
cl <- classIntervals(covid$casesw, 5)

covid <- mutate(covid, casesw_class = cut(casesw, round(cl$brks), include.lowest = TRUE))

ggplot(wld) + 
  geom_sf(fill = "grey93", colour = "white", size = .2) +
  geom_sf(data = covid, aes(fill = casesw_class), 
          shape = 21, alpha = .3, colour = alpha("black", .5), size = 2) +
  geom_sf(data = grid, size = .2, linetype = "dashed", colour = "grey50", alpha = 0.5) +
  scale_fill_brewer(palette = "RdPu", labels = c("< 99", "99 - 213", "213 - 414", "414 - 881", "> 881")) +
  coord_sf(crs = "+proj=robin +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs") +
  theme_void() +
  theme(legend.position = c(0.15, 0.38), 
        legend.title = element_text(vjust = 0.3,
                                    hjust = 1.1)) +
  labs(fill = "COVID cases")

ggsave("covid_cases_map.png")

## Re

ggplot(covid, aes(r.mean)) + geom_density()

cl <- classIntervals(covid$r.mean, 5, "jenks")

covid <- mutate(covid, r.mean_class = cut(r.mean, round(cl$brks, 2), include.lowest = TRUE))

ggplot(wld) + 
  geom_sf(fill = "grey93", colour = "white", size = .2) +
  geom_sf(data = covid, aes(fill = r.mean_class), 
          shape = 21, alpha = .3, colour = alpha("black", .5), size = 2) +
  geom_sf(data = grid, size = .2, linetype = "dashed", colour = "grey50", alpha = 0.5) +
  scale_fill_brewer(palette = "YlGnBu", labels = c("< 1.1", "1.1 - 1.4", "1.4 - 1.5", "1.5 - 1.7", "> 1.7")) +
  coord_sf(crs = "+proj=robin +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs") +
  theme_void() +
  theme(legend.position = c(0.15, 0.38), 
        legend.title = element_text(vjust = 0.3,
                                    hjust = .5)) +
  labs(fill = expression(R[e]))

ggsave("Re_map.png")

## Tmean
ggplot(covid, aes(tmean)) + geom_density()

cl <- classIntervals(covid$tmean, 5, "jenks")

covid <- mutate(covid, tmean_class = cut(tmean, round(cl$brks, 2), include.lowest = TRUE))

ggplot(wld) + 
  geom_sf(fill = "grey93", colour = "white", size = .2) +
  geom_sf(data = covid, aes(fill = r.mean, size = tmean), 
          shape = 21, alpha = .3, colour = alpha("black", .5)) +
  geom_sf(data = grid, size = .2, linetype = "dashed", colour = "grey50", alpha = 0.5) +
  # scale_fill_brewer(palette = "RdYlBu", direction = -1, labels = c("< 2.8", "2.8 - 8.5", "8.5 - 13.1", "13.1 - 19.7", "> 19.7")) +
  coord_sf(crs = "+proj=robin +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs") +
  theme_void() +
  theme(legend.position = c(0.15, 0.38), 
        legend.title = element_text(vjust = 0.3,
                                    hjust = 0)) +
  labs(fill = "Mean temperature (ºC)")
