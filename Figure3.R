...
abs
conf<-data[,c("tmean","rh","ah","uv","oxgov10r","calyear","r.mean", "lat", "long")]
...
plot(data$calyear, data$lat)
plot(data$calyear, data$long)
...
a<-var(predict(model0))
b<-model0$Psi$countrycode
c<-model0$Psi$mcccitiescode
d<-(mean((datar$r.var)))
(as.numeric((a/(a+b+c+d))*100))
...
a<-var(predict(model1))
b<-model1$Psi$countrycode
c<-model1$Psi$mcccitiescode
d<-(mean((datar$r.var)))
(as.numeric((a/(a+b+c+d))*100))
(as.numeric((a/(a+b+c))*100))
# MODEL 1
...
a<-var(predict(model3))
b<-model3$Psi$countrycode
b<-0.0315
c<-model1$Psi$mcccitiescode
c<-0.0088
d<-(mean((datar$r.var)))
...
# EXCLUDING CITY LEVEL COVARIATES
...
vardistr <- dplyr::select(datar, tmean, rh, ah, oxgov10r) %>% 
