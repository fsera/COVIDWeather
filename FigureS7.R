...
fixed <- formula(r.mean~1+log(population)+log(density)+(oldpop)+(log(gdp))+log(pm25))
...
fixed <- formula(r.mean~1+ns(rh,df=3)+log(population)+log(density)+(oldpop)+(log(gdp))+log(pm25))
...
fixed <- formula(r.mean~1+ns(ah,df=4)+log(population)+log(density)+(oldpop)+(log(gdp))+log(pm25))
...
fixed <- formula(r.mean~1+ns(uv,df=2)+log(population)+log(density)+(oldpop)+(log(gdp))+log(pm25))
...
fixed <- formula(r.mean~1+ns(ws,df=2)+log(population)+log(density)+(oldpop)+(log(gdp))+log(pm25))
...
fixed <- formula(r.mean~1+prectot+log(population)+log(density)+(oldpop)+(log(gdp))+log(pm25))
...
p1 <- filter(df, panel == "A") %>% ggplot() + 
  ...
  labs(y = expression(R[e]~"difference"), x = "Mean temperature (ºC)", title = "p = 0.005") +
...
p2 <- filter(df, panel == "B") %>% ggplot() + 
  ...
  labs(y = expression(R[e]~"difference"), x = "Relative humidity (%)", title = "p = 0.055") +
  theme_bw() +
  theme(strip.text = element_text(hjust = 0.08, size = 10),
...
p3 <- filter(df, panel == "C") %>% ggplot() + 
  ...
  #scale_color_manual(values = c("#cb181d", "#08519c", "#08519c", "#ae017e")) +
  scale_y_continuous(limits = c(-0.2, .2)) +
  labs(y = expression(R[e]~"difference"), x = expression("Absolute humidity (g/"*m^3*")"), title = "p = 0.001") +
  theme_bw() +
  theme(strip.text = element_text(hjust = 0.08, size = 10),
        plot.title = element_text(face = "plain", size = 10))
...
p4 <- filter(df, panel == "D") %>% ggplot() + 
  ...
  scale_y_continuous(limits = c(-0.2, .2)) +
  labs(y = expression(R[e]~"difference"), x = expression("Solar surface radiation (J/"*m^2*")"), title = "p = 0.001") +
  theme_bw() +
  theme(strip.text = element_text(hjust = 0.08, size = 10),
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
...
ggsave("FigureS7.png", width = 5, height = 5)
